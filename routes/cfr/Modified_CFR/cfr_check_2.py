...
def getaction_v3(num_action, strategy, parentNode, return_reward):
    # print("get action")
    a = 0
    cum_prob = 0
    r = random.uniform(0, 1)
    while True:
        if a > num_action - 2:
            break
        if a == parentNode:
            cum_prob = cum_prob + return_reward
        else:
            cum_prob = cum_prob + strategy[a]
        if r < cum_prob:
            break
...
def getreward_v4(action,topology,strategy,default_strategy,currentNode,targetNode):
    ...
    if currentNode == targetNode:
        ...
        while currentNode != targetNode:
            ...
            action = getaction_v3(len(currentStrategy), currentStrategy[:], parentNode, strategy[parentNode][currentNode])
            chosenPath.append(action)
            reward = reward + topology[currentNode][action]
            # parentNode = chosenPath[len(chosenPath) - 2]
            if parentNode == action:
                if action == chosenPath[0]:
                    return 0
                else:
                    parentNode = chosenPath[len(chosenPath) - 3]
            currentNode = action
            if currentNode == targetNode:
                reward = 1 / reward
                break
...
def getreward_v4_2(action,topology,strategy,default_strategy,currentNode_,targetNode):
    reward = 0
    # chosenPath = path[:] #visited nodes
    chosenPath = [currentNode_, action]

    # chosenPath.append(action)
    options = [currentNode_, action] #keep track of the current path
    reward = reward + topology[chosenPath[len(chosenPath) - 2]][action]
