            ...
            try:
                ...
                if cs:
                    ...
                    if res:
                        ...
                        json_object = dumps(res,indent=4)
                        with open('routes/ShotHound/sample.json', "w") as outfile:
...
def parse_args():
    ...
    parser.add_argument("domain", default="TESTLAB.LOCAL", help="a FQDN", type=str)
    parser.add_argument("--dbuser",dest="dbuser",default="neo4j",help="neo4j db user name",type=str)
    parser.add_argument("--dbpass", dest="dbpass", default="Longnhu2201", help="neo4j db password", type=str)
    ...
    try:
        ...
        if sh.connect():
            if sh.find_logical_paths(srcname=args.source,trgtname=args.target):
                print(len(sh.get_logical_paths()))
                data ={
                    'Logical': len(sh.get_logical_paths()),
                }
                with open('routes/ShotHound/finalInfo.json', 'w') as outfile:
                ...
                with open('finalInfo.json', 'w') as outfile:
                    json.dump(data, outfile)
                logger.info(f'---------------------------------------')
